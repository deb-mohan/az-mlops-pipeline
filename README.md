...
On first run, the script generates `infrastructure/terraform/backend.tf` with the randomized storage account name. On subsequent runs, it reads the existing `backend.tf` and verifies the resources exist in Azure.
...
make bootstrap LOCATION=westus2
```
...
Resources follow the Azure [Cloud Adoption Framework (CAF)](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/resource-naming) prefix pattern: `<resource-type>-<project>-<environment>`

Examples:
- Resource Group: `rg-iemlops-dev`
- ML Workspace: `mlw-iemlops-dev`
- Application Insights: `appi-iemlops-dev`
- Key Vault: `kv-iemlops-dev-a1b2c3` (with random suffix, max 24 chars)
- Storage Account: `stiemlopsdeva1b2c3` (no hyphens, random suffix, max 24 chars)
...
│       ├── backend.tf           # Backend configuration (generated by bootstrap)
│       ├── modules/
